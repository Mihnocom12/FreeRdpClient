...
{
	if (tpkt_verify_header(s)) //[MS-RDPBCGR 194]
...
int rdp_recv_callback(rdpTransport* transport, wStream* s, void* extra)
{
	...
	WLog_ERR(TAG, "********%s : rdp->state:%d", __FUNCTION__, rdp->state);
	...
	switch (rdp->state)
	{
		...
		case CONNECTION_STATE_MCS_CONNECT:
			WLog_ERR(TAG, "---file:%s,func: %s : rdp->state:%d",__FILE__, __FUNCTION__, rdp->state);
		...
		case CONNECTION_STATE_LICENSING:
			WLog_ERR(TAG, "--%s : rdp->state:LICENSING", __FUNCTION__);
