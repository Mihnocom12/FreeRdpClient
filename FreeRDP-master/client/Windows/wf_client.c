...
BOOL wf_post_connect(freerdp* instance)
{
	...
	if (settings->SoftwareGdi)
	{
		...
		if (!gdi_init(instance, CLRCONV_ALPHA | 64, wfc->primary->pdata))
			return FALSE;
	...
	else if (settings->ServerPort == RDP_PORT)
...
void* wf_input_thread(void* arg)
{
	...
	while (MessageQueue_Wait(queue))
	{
		...
		{
			WLog_ERR(TAG, "--%s received msg id:%d", __FUNCTION__, message.id);
...
int wfreerdp_client_start(rdpContext* context)
{
	...
	wfc->wndClassName = _tcsdup(_T("ZHUFreeRDP"));
