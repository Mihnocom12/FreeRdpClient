	...
	if (wfc != NULL)
	{
		...
		switch (Msg)
		{
			...
			case WM_PAINT:
				...
				wf_scale_blt(wfc, hdc, x, y, w, h, wfc->primary->hdc, x - wfc->offset_x + wfc->xCurrentScroll, y - wfc->offset_y + wfc->yCurrentScroll, SRCCOPY,hWnd);
...
BOOL SaveBitmapToFile(LPCTSTR lpszFilePath, HBITMAP hBm,HDC hDC)
{
	//  定义位图文件表头
...
void Print_Screen(HWND hwnd, HDC hDC)
{
	...
	sprintf(str, "D:\\freerdp\\pic\\%d.bmp", count++);
	SaveBitmapToFile(str, hBitMap,hDC);
}

BOOL wf_scale_blt(wfContext* wfc, HDC hdc, int x, int y, int w, int h, HDC hdcSrc, int x1, int y1, DWORD rop,HWND hwnd)
{
	int ww, wh, dw, dh;
